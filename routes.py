...
def recommend_(features,feature_list):
    neighbors = NearestNeighbors(n_neighbors=12, algorithm='brute', metric='cosine')
...
@app.route('/')
def index():
    df=pd.read_csv('D:/madhur/Projects/frs/images.csv',usecols=['filename'])
...
def recommend(image_name):
    features = feature_extraction(os.path.join("D:/madhur/Projects/frs/static/images",image_name),model)
